...
export async function parseEpubMetadata(file: File): Promise<Partial<BookMetadata>> {
  ...
  // 1. Find container.xml to get the path of the OPF file (case-insensitive check)
  let containerFile = loadedZip.file("META-INF/container.xml");
  if (!containerFile) {
    const files = Object.keys(loadedZip.files);
    const foundKey = files.find(f => f.toLowerCase() === "meta-inf/container.xml");
    if (foundKey) {
      containerFile = loadedZip.file(foundKey);
    }
  }
  ...
  // 2. Parse OPF file for metadata (case-insensitive & fallback strategy check)
  let opfFile = loadedZip.file(opfPath);
  if (!opfFile) {
    const files = Object.keys(loadedZip.files);
    const normalizedOpfPath = opfPath.replace(/\\/g, "/").trim().toLowerCase();
    const foundKey = files.find(f => f.replace(/\\/g, "/").trim().toLowerCase() === normalizedOpfPath) || 
                     files.find(f => f.toLowerCase().endsWith(".opf"));
    if (foundKey) {
      console.log(`[EPUB Parser] Localizado OPF por chave secundária ou case-insensitive: "${foundKey}"`);
      opfFile = loadedZip.file(foundKey);
    }
  }
  ...
  // 3. Try to find cover image safely
  ...
  if (coverItem) {
    ...
    if (coverPath) {
      ...
      let coverFile = loadedZip.file(fullCoverPath);
      if (!coverFile) {
        // Fallback: look for a file ending with the cover's base name
        const coverFileName = coverPath.split("/").pop()?.toLowerCase();
        if (coverFileName) {
          const files = Object.keys(loadedZip.files);
          const foundKey = files.find(f => f.toLowerCase().endsWith("/" + coverFileName) || f.toLowerCase() === coverFileName);
          if (foundKey) {
            coverFile = loadedZip.file(foundKey);
          }
        }
      }
      if (coverFile) {
        try {
          ...
          console.warn("[EPUB Parser] Erro obtendo blob do arquivo de capa:", covErr);
        }
      }
    }
  }
