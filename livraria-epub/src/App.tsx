  ...
  return (
    <>
      ...
      <div className="p-4 pt-2 border-t border-gray-800 bg-black/40 flex flex-col gap-2 shrink-0 safe-pb">
        ...
        <button 
          onClick={() => book.inTrash ? alert("Para remover permanentemente, esvazie a lixeira.") : onMoveToTrash(book.id)}
          className={cn(
...
export default function App() {
  ...
  const addFolder = async () => {
    try {
      ...
      if (!window.showDirectoryPicker) {
        alert("Seu navegador não suporta a seleção de pastas. Tente usar o Chrome no Desktop.");
        return;
      }
  ...
  const addFiles = async () => {
    try {
      ...
      if (!window.showOpenFilePicker) {
        alert("Seu navegador não suporta a seleção de arquivos. Tente usando o Chrome no Desktop.");
        return;
      }
  ...
  const enrichBook = async (bookId: string) => {
    ...
    try {
      ...
      alert("Erro ao buscar metadados com IA. Verifique sua chave API e conexão.");
    } finally {
  ...
  const enrichAllBooks = async () => {
    ...
    if (booksToEnrich.length === 0) {
      alert("Todos os livros já possuem metadados ou estão na lixeira.");
      return;
    }

    if (!confirm(`Deseja sincronizar ${booksToEnrich.length} livros usando IA?`)) return;
    ...
    setIsEnriching(false);
    alert(`${successCount} livros foram sincronizados com sucesso.`);
  };
