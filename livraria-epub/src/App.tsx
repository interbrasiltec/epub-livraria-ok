...
import { FolderPlus, RefreshCw, Search, Book, BookOpen, Clock, Filter, ChevronRight, X, Info, Trash2, RotateCcw, Star, Menu } from "lucide-react";
          ...
          {coverSrc ? (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {book.coverUrl && !book.coverData && (
                <button 
                  onClick={onEnrich}
                  className="text-[10px] bg-white/5 hover:bg-white/10 text-gray-400 px-2 py-1 rounded transition-colors flex items-center gap-1"
                >
                  <RefreshCw size={10} />
                  Tentar outra capa
                </button>
...
export default function App() {
  ...
  const addFiles = async () => {
    try {
      ...
      if (!window.showOpenFilePicker) {
        showToastMsg("Seu navegador não suporta a seleção de arquivos. Tente usando o Chrome no Desktop.", "error");
        return;
      }
        ...
        if (!exists) {
          try {
            ...
            foundBooks.push({
              ...
              id: crypto.randomUUID(),
              fileName: name,
              filePath: name, // For single files, path is just name
  ...
  const enrichBook = async (bookId: string) => {
    if (isEnriching) {
      console.log("Gemini request bloqueada por sincronização em andamento");
      console.warn("[Sync] Sincronização já em andamento. Ignorando clique duplicado.");
      return;
    }
    ...
    console.log(`[AI Request Counter] Sincronização iniciada para "${book.title}". Requisições feitas nesta sessão: ${syncCountRef.current}`);

    try {
      console.log("Gemini request iniciada:", book.title);
      ...
      if (activeApiKey) {
        ...
        if (resp.ok) {
          try {
            ...
            throw new Error(e.message || "Erro de resposta da IA (JSON inválido).");
          }
        } else {
          try {
            const errObj = JSON.parse(responseText.trim());
            throw new Error(errObj.error || errObj.details || `Erro do servidor (${resp.status})`);
          } catch {
            if (resp.status === 404) {
              throw new Error("Erro 404: O modelo ou serviço de IA não foi encontrado no servidor. O servidor pode estar usando uma versão antiga do modelo.");
            }
            if (resp.status === 429) {
              throw new Error("Limite de requisições excedido (Rate Limit / 429). Aguarde alguns instantes.");
            }
            throw new Error(`O servidor retornou um status de erro (${resp.status}). A API pode estar temporariamente fora do ar.`);
          }
        }
      }

      console.log("Gemini response recebida");
      console.log(`[Sync] Successfully enriched ${book.title}:`, data);
      const now = Date.now();
      ...
      console.error(`[Sync] Critical error for ${book.title}:`, err);
      const msg = err.message || "";
      if (msg.toLowerCase().includes("api key") || msg.toLowerCase().includes("expired") || msg.toLowerCase().includes("invalid_argument") || msg.toLowerCase().includes("key_invalid")) {
        showToastMsg("A chave de API do Gemini falhou ou expirou. Forneça uma chave de API pessoal na barra lateral esquerda (Configuração IA).", "error");
        setIsSidebarOpen(true);
      } else if (msg.toLowerCase().includes("404")) {
        showToastMsg(`Erro 404 ao sincronizar "${book.title}": Modelo ou recurso não encontrado na API Gemini. Certifique-se de que o modelo fornecido é válido e suportado pela sua chave de API.`, "error");
      } else {
  ...
  const enrichAllBooks = async () => {
    if (isEnriching) {
      console.log("Gemini request bloqueada por sincronização em andamento");
      console.warn("[Sync] Sincronização em lote já em andamento. Ignorando clique duplicado.");
      return;
    }
    ...
    if (!confirm(`Deseja sincronizar ${booksToEnrich.length} livros usando Inteligência Artificial?`)) return;

    setIsEnriching(true);
    let successCount = 0;
    let apiExpiredErrorHandled = false;

    for (const book of booksToEnrich) {
      try {
        console.log("Gemini request iniciada:", book.title);
        // Safety delay of 2 seconds before each request to stay well below Gemini rate limits
        await new Promise(resolve => setTimeout(resolve, 2000));

        syncCountRef.current++;
        console.log(`[AI Request Counter] Sincronização em lote iniciada para "${book.title}". Sequencial #${successCount + 1}. Requisições feitas nesta sessão: ${syncCountRef.current}`);
        ...
        if (activeApiKey) {
          ...
          if (resp.ok) {
            ...
            try {
              ...
              if (resp.status === 404) {
                throw new Error("Erro 404: O modelo ou serviço de IA não foi encontrado.");
              }
              if (resp.status === 429) {
                throw new Error("Limite de requisições excedido (Rate Limit / 429).");
              }
              throw new Error(`O servidor retornou um status de erro (${resp.status}).`);
            }
          }
        }
        ...
        const msg = err.message || "";
        if (!apiExpiredErrorHandled && (msg.toLowerCase().includes("api key") || msg.toLowerCase().includes("expired") || msg.toLowerCase().includes("invalid_argument") || msg.toLowerCase().includes("key_invalid"))) {
          showToastMsg("A sincronização falhou porque a chave de API falhou ou expirou. Forneça uma chave de API própria na barra lateral esquerda (Configuração IA).", "error");
          setIsSidebarOpen(true);
          apiExpiredErrorHandled = true;
          break;
        } else if (msg.toLowerCase().includes("404")) {
          showToastMsg(`Erro 404 ao sincronizar "${book.title}": Modelo ou recurso não encontrado na API Gemini. Certifique-se de que o modelo possui suporte sob sua chave de API.`, "error");
        } else {
          showToastMsg(`Erro ao sincronizar "${book.title}": ${err.message}`, "error");
        }
        // "não chamar IA novamente automaticamente após erro" -> Quebramos o loop imediatamente no primeiro erro do lote
        console.warn("[Sync] Sincronização em lote interrompida devido a erro para evitar chamadas adicionais automáticas.");
        break;
      }
    }
