...
import { normalizeSearchTerm } from "./services/bookApi";
  ...
  const enrichBook = async (bookId: string) => {
    ...
    try {
      ...
      try {
        ...
        console.error(`[Sync] Failed to parse JSON response. raw: ${responseText.substring(0, 500)}`);
        throw new Error(`Resposta inválida do servidor (HTML/Texto recebido em vez de JSON).`);
      }

      if (!resp.ok) {
        const errorMsg = data.error || data.details || `Erro do servidor (${resp.status})`;
        console.error(`[Sync] API Error for ${book.title}:`, { status: resp.status, data });
